...
const TakeAwayChart = ({ ordersData }) => {
  ...
  useEffect(() => {
    ...
    if (chartData.labels && chartData.labels.length > 0) {
      const ctx = document.getElementById("takeawaychart").getContext("2d");
  ...
  return (
    <div>
     ...
     {chartData?.labels?.length === 0 ? (
        ...
        <canvas id="takeawaychart" width="400" height="200"></canvas>
      )}
    </div>
  );
};

export default TakeAwayChart;
